...
#
# 💡 幂律分布常用路径示例：
#   cache_dir = "power_law_cache"           # 默认路径 (幂律分布专用)
#   cache_dir = "powerlaw_analysis_cache"   # 自定义幂律缓存
#   cache_dir = "scan_results/power_law"    # 扫描结果子目录
#   cache_dir = "my_powerlaw_analysis"      # 自定义路径
#   cache_dir = r"D:\research\powerlaw"     # 绝对路径 (Windows)
#   cache_dir = "/home/user/powerlaw"       # 绝对路径 (Linux/Mac)

# ✅ 缓存目录设置 - 只需修改这一行！
cache_dir = "scan_results/power_law_cache"
...
def detect_multiple_peaks(r_values, xi_values, main_peak_r, prominence_threshold=0.6):
    ...
    - xi_values: 关联长度数组
    - main_peak_r: 主峰位置
    - prominence_threshold: 次峰与主峰的高度比阈值（默认0.6，即60%）
    ...
    if main_peak_in_detected is None:
        ...
        for i in range(1, len(sorted_heights)):
            ...
            # 如果有峰值达到主峰的阈值以上，认为是多峰
            if ratio >= prominence_threshold:
                is_multi_peak = True
    ...
    # 添加次峰详细信息
    secondary_peak_details = []
    if len(secondary_peaks) > 0:
        for sec_idx in secondary_peaks:
            sec_height = xi_values[sec_idx]
            sec_position = r_values[sec_idx]
            sec_ratio = sec_height / main_peak_height
            secondary_peak_details.append({
                'index': sec_idx,
                'position': sec_position,
                'height': sec_height,
                'ratio_to_main': sec_ratio
            })
    ...
    peak_info = {
        ...
        'secondary_peak_details': secondary_peak_details,
        'detection_threshold': prominence_threshold,
        'problem_type': problem_type,
        'local_smoothness': local_smoothness,
        'method': 'improved_statistical_analysis',
        'is_reliable': True
    }
