...
# ==================== 📁 缓存路径配置区域 ====================
# 📝 使用说明：只需修改下面的 cache_dir 变量即可切换缓存目录
#
# 💡 常用路径示例：
#   cache_dir = "power_law_cache"           # 默认路径 (兼容原始代码)
#   cache_dir = "poisson_analysis_cache"    # 泊松分布专用
#   cache_dir = "powerlaw_analysis_cache"   # 幂律分布专用
#   cache_dir = "my_custom_analysis"        # 自定义路径
#   cache_dir = r"D:\research\cache"        # 绝对路径 (Windows)
#   cache_dir = "/home/user/analysis"       # 绝对路径 (Linux/Mac)

# ✅ 缓存目录设置 - 只需修改这一行！
cache_dir = "scan_results/poisson_law_cache"

print(f"📁 缓存目录: {cache_dir}")

# 动态设置缓存路径到analyze_power_law_symmetry模块
import src.analyze_symmetry as aps

# 重新配置模块的缓存路径
aps.CACHE_DIR = cache_dir
aps.DATA_CACHE_DIR = os.path.join(aps.CACHE_DIR, "correlation_data")
aps.ANALYSIS_CACHE_DIR = os.path.join(aps.CACHE_DIR, "analysis_results")
...
def collect_all_scan_results(distribution_filter=None):
    """
    收集扫描结果 - 支持泊松分布和幂律分布
    ...
    - distribution_filter: 分布类型过滤器
      ...
      - 'powerlaw': 只收集幂律分布结果 (gamma_pref!=None)
    ...
    if distribution_filter is None:
        ...
        print("📊 收集泊松分布扫描结果...")
    elif distribution_filter == 'powerlaw':
        print("📊 收集幂律分布扫描结果...")
    else:
...
if __name__ == "__main__":
    ...
    if 'poisson' in cache_dir_lower:
        suggested_filter = 'poisson'
        print(f"🔍 根据缓存目录名称，建议分析：🔵 泊松分布")
    elif 'powerlaw' in cache_dir_lower or 'power_law' in cache_dir_lower:
        if 'power_law_cache' == cache_dir:
            suggested_filter = None  # 可能包含混合数据
            print(f"🔍 检测到默认缓存目录，将分析：🎯 所有分布类型")
        else:
            suggested_filter = 'powerlaw'
            print(f"🔍 根据缓存目录名称，建议分析：🔴 幂律分布")
    else:
        suggested_filter = None
        print(f"🔍 未能识别分布类型，将分析：🎯 所有分布类型")
    ...
    if len(results_list) == 0:
        if suggested_filter:
            filter_names = {'poisson': '泊松分布', 'powerlaw': '幂律分布'}
            filter_name = filter_names.get(suggested_filter, suggested_filter)
            print(f"❌ 没有找到任何{filter_name}扫描结果")
        else:
            print("❌ 没有找到任何扫描结果")
        exit()
